...
def get_concentric_ring_value(node: Node):
    match node:
        case node if node.left and node.right:
            return '2'
        case node if node.left:
            return '-1'
        case node if node.right:
            return '1'
        case _:
            return '0'
...
def concentric_rings(avl: AVLTree):
    data = {
        'max_level': 0,
        'level_nodes': [[]]
    }
    get_level_leafs(data, avl.root)
    acc = ''
    for level in data['level_nodes']:
        for node in level:
            acc += f'.{get_concentric_ring_value(node)}'
    return acc[1:]

    
    
def get_level_leafs(data, node, level = 0):
    global max_level
    global levelNodes
    if level > data['max_level']:
        data['level_nodes'].append([])
        data['max_level'] = level
    data['level_nodes'][level].append(node)
    if node.left:
        get_level_leafs(data, node.left, level + 1)
    if node.right:
        get_level_leafs(data, node.right, level + 1)

        
while input() != '0':
    avl = AVLTree()
    list(map(lambda a: avl.insert(int(a)), input().split() ))
    print(concentric_rings(avl))
