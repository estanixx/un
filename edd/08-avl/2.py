...
def how_many_leafs(avl: AVLTree):
    data = {
        'max_level': 0,
        'level_nodes': []
    }
    get_top_level_leafs(data, avl.root)
    return len(data['level_nodes'])
    
def get_top_level_leafs(data, node, level = 0):
    global max_level
    global levelNodes
    ...
    if level == data['max_level']:
        data['level_nodes'].append(node)
    if node.left:
        get_top_level_leafs(data, node.left, level + 1)
    if node.right:
        get_top_level_leafs(data, node.right, level + 1)
        
for _ in range(int(input())):
    avl = AVLTree()
    list(map(lambda a: avl.insert(int(a)), input().split()[:-1]))
    print(how_many_leafs(avl))
